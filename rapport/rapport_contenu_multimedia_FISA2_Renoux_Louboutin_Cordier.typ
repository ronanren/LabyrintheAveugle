...
Dans le cadre du module Contenu Multimédia, nous avons réalisé un projet d'application de jeu. Il s'agit d'un labyrinthe aveugle. L'objectif du jeu est de parcourir le labyrinthe en partant d'un coin pour atteindre le coin inverse en diagonale. Cependant, le joueur doit évoluer dans le noir sans lumière pour se repérer. Il ne va utiliser que des sons. En fonction de son emplacement et des murs environnants, le joueur obtient un retour sonore dans un espace 3D.
(Exemple : si le joueur se trouve à gauche d'un mur, il entend un son à sa gauche.)
Le joueur devient donc capable de se repérer dans le labyrinthe grâce à ses oreilles. Une musique victorieuse s'enclenche dès lors que le joueur arrive à trouver l'objectif.

= Mise en oeuvre

== Création du Labyrinthe

Le labyrinthe du jeu est généré selon l'algorithme Sidewinder. Concrètement, il s'agit d'un algorithme de génération de labyrinthes aléatoires. Il consiste à créer un labyrinthe en partant d'une grille de cubes. La première ligne de la grille est remplie de murs, puis on parcourt les lignes suivantes. Pour chaque ligne, on réalise des groupes de cubes pouvant aller de 1 cubes à N cubes (avec N le nombre de cubes par ligne). Puis, dans chaque groupe de cubes, nous supprimons un seul mur du haut au hasard parmi les cubes. Ainsi, on obtient un labyrinthe dans lequel il n'y a aucune zone inaccessible. Pour chaque cube du labyrinthe, nous enregistrons la configuration ouverte ou fermée des murs. Ces informations nous seront utiles pour la gestion des collisions et pour la gestion des sources sonores.
...
Initialement la vision en profondeur maximale de la caméra est de 1. Cette valeur très faible et égale à la vision en profondeur minimale. Cela permet d'obtenir un écran complètement noir pour jouer. En mode debug, cette valeur est de 1000. Cela permet de voir la scène en entière et donc de se repérer.
Le mode debug est accessible en appuyant sur la touche *"Y"* du clavier. Avec ce mode, nous obtenons une vue du dessus du labyrhinthe. 
...
Ce mode debug nous a notamment permis de vérifier le bon fonctionnement de l'algorithme de génération du labyrinthe. Celui-ci pourrait également nous être utile pour mettre en place le mode spectateur du jeu.
